...
from util import err, dbg, enumerate_descendants, make_uuid, uhoextract
# import pout pout.inject()
# import pout
# pout.inject()
...
class Notebook(Container, Gtk.Notebook):
    ...
    def configure(self):
        ...
        # FIXME should we deal with old configs?
        if self.config['tab_position'].lower() == 'hidden':
            self.config['tab_position'] = 'top'
            self.config['tabs_hidden'] = True
...
#        self.modify_style(style)
    ...
    def relayout(self, layout):
        ...
        for tab in xrange(0, self.get_n_pages()):
            ...
            clcfg = child.config_section # its relayout, child must have one
            if hasattr(child, 'relayout'):
                child.relayout(layout)
            if hasattr(child, 'caption') and child.caption != '':
                label.set_custom_label(child.caption)
            else:
                label.clear_custom()
    ...
    def describe_layout(self, parent, global_layout):
        ...
        layout  = {}
        name = "%sn" % self.config.name_next()
        self.config_section = name
        dbg('~DESCRIBE  [Notebook:%s of %s]' % (name, parent))
        ...
        for tab in xrange(0, self.get_n_pages()):
            child = self.get_nth_page(tab)
            label = self.get_tab_label(child)
            if hasattr(child, 'inPaned'):
                child.inPaned = False
            if hasattr(child, 'caption'):
                child.caption = label.get_custom_label()
    ...
    def create_layout(self, layout):
        ...
        dbg('CREATE layout for Notebook:%s of %s.' % (self.config_section, layout['parent']))
        ...
        if len(children) <= 1:
            ...
            return
        keys = children.keys()
        keys.sort() # lexical is 'nuff
        num = 0
        for child_key in keys:
            clayout = children[child_key]
            dbg('CREATE child %s:%s in Tab%d.' % (clayout['type'], child_key, num))
            if clayout['type'] == 'Terminal':
                pass
            elif clayout['type'] == 'VPaned':
                page = self.get_nth_page(num)
                self.split_axis(page, True)
            elif clayout['type'] == 'HPaned':
                page = self.get_nth_page(num)
        ...
        for child_key in keys:
            ...
            if not page:
                ...
                self.newtab(clayout)
                page = self.get_nth_page(num)
            if clayout.has_key('caption'):
                caption = clayout['caption']
                if caption:
                    label = self.get_tab_label(page)
                    label.set_custom_label(caption)
                    dbg('SETCAP %s for %s of Tab%d' % (caption, child_key, num))
                    page.caption = caption
                else:
                    dbg('SETCAP NONE for %s of Tab%d' % (child_key, num))
            else:
                dbg('SETCAP NOKEY for %s of Tab%d' % (child_key, num))
            page.config_section = child_key
            page.create_layout(clayout)
            self.last_active_term[page] = page.uuid
            num = num + 1
        ...
        dbg('XXY END layout for Notebook:%s of %s.' % (self.config_section, layout['parent']))

    def split_axis(self, widget, vertical=True, cwd=None, sibling=None, widgetfirst=True):
        """Split the axis of a terminal inside us"""
        dbg('~SPLIT PAgE: %s of %s [%s]' % (widget.config_section, self.config_section, sibling))
        order = None
        ...
        capt = 'NC'
        fromcfg = self.config.get_term_config(self.config_section, widget)
        ...
        # caption order: already set, widget's, sibling's, 'NC'
        capt = label.get_custom_label()
        if capt != '': # user set on tab
            pass
        elif fromcfg['caption'] != '': # widget's
            capt = fromcfg['caption']
        elif sibling and sibling.caption != '':
            capt = sibling.caption
        else:
            capt = 'NC'
        fromcfg['caption'] = capt
        label.set_custom_label(capt)
        ...
        if not sibling:
            ...
            # FIXME we're reordering, why we move profile here?
            sibling.force_set_profile(None, widget.get_profile())

        container.caption = capt # pass caption down the tree
        sibling.caption = capt
        widget.caption = capt
        sibling.inPaned = True
        widget.inPaned = True
        ...
        GObject.idle_add(child.ensure_visible_and_focussed)
    ...
    # 1st call from init with widget, 2nd and next calls from top_window are with profile
    def newtab(self, debugtab=False, widget=None, cwd=None, metadata=None, profile=None):
        ...
        if not widget:
            ...
            if cwd:
                widget.set_cwd(cwd)
            if profile:
                profile['issue'] = 'This is your new tab you asked for.'
                capt = profile['caption']
                if capt and capt != '' and not capt.endswith('.new'):
                    profile['caption'] = "%s.new" % capt
            widget.apply_new(profile)
        ...
        elif profile and profile['_tabnext']:
            tabpos = profile['_tabnext']
        else:
            tabpos = -1
        ...
        elif profile and profile['caption']:
            label.set_custom_label(profile['caption'])
        elif widget.caption:
            label.set_custom_label(widget.caption)
        label.connect('close-clicked', self.closetab)
        label.connect('edit-done', self.on_edit_done)
        label.show_all()
        widget.show_all()
    ...
    def update_tab_label_text(self, widget, text):
        ...
        if not label:
            ...
            return
        #dbg('~CHANGE TAB LABEL to %s' % text)
        label.set_label(uhoextract(text, smart=True))

    def on_edit_done(self, widget):
        capt = ''
        if hasattr(widget, 'notebook'):
            num = widget.notebook.get_current_page()
            child = widget.notebook.get_nth_page(num)
            capt = widget.get_custom_label()
            if hasattr(child, 'set_caption'):
                child.set_caption(capt)
                dbg('~EDIT_DONE! Set caption "%s" on %s at tab: %d' % (capt, child.config_section, num))
        else:
            dbg('~EDIT_DONE! Got unusable widget %s' % widget)
    ...
    # FIXME this does NOT work called wit TabLabel object.
    ...
    ## FIXME Not all tabs have terminals. Some have paned container. Then we fail with
    ## Err: notebook.py", line 487, in do_deferred_on_tab_switch
    ## Err: notebook.py", line 496, in on_tab_switch
    ## AttributeError: 'NoneType' object has no attribute 'ensure_visible_and_focussed'
    ## WontFix. It does not bite hard and only once at start.
    ...
    ## FIXME Not all tabs have terminals. Some have paned container. We should
    ## descend here to the first child 'ensure_visible_and_focussed' capable
    def on_tab_switch(self, notebook, page,  page_num,  data=None):
        ...
        if tabs_last_active_term:
            ...
            if term and hasattr(term, "ensure_visible_and_focussed"):
...
class TabLabel(Gtk.HBox):
    ...
    def __init__(self, title, notebook):
        ...
        self.label.connect('edit-done', self.on_label_done)
        self.update_angle()
    ...
    def clear_custom(self):
        ...
        self.label.set_custom(False)

    def set_custom_label(self, text):
        """Set a permanent label as if the user had edited it"""
        self.label.set_edit_base(text)
        self.label.set_custom()

    def get_custom_label(self):
        """Return a custom label if we have one, otherwise empty"""
        return(self.label.get_custom_text()) # returns '' if not custom

    def edit(self):
        self.label.edit()

    def on_label_done(self, widget):
        """Re-emit for parent notebook"""
        self.emit('edit-done')

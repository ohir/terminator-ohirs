...
DEBUG = False
# set this to true to show only messages that start with ~
DEBUGTILDE = True
...
ourhost = None # set once
def uhoextract(instr='', user=True, host=True, smart=None):
    """Get user@host part of conventional os prompt (at title-change)"""
    r = 'user@host'
    m = re.search(r'[a-z._A-Z]+@[a-zA-Z]+', instr)
    if m:
        r = m.group()
    if not r:
        r = 'user@host'
    if user and host and not smart: # user@host11
        return r
    ...
    if smart:
        ...
        if not ourhost:
            ourhost = os.uname()[1]
        # return user for host == ourhost and host, if host != ourhost
        if h == ourhost:
            return u
        if u != 'root':
            return h
        else:
            return "R@%s" % h.upper() # indicate root session
    elif user:
        return u
    elif host:
        return h
...
def dbg(log = ""):
    ...
    if DEBUG:
        ## local print-debug: add tilde as the first character
        if DEBUGTILDE and not log[0] == '~':
            return
...
terminator_config_dir = None
def get_config_dir():
    global terminator_config_dir
    if terminator_config_dir:
        return terminator_config_dir
    ...
    if DEBUG:
        terminator_config_dir = os.path.join(configdir, 'termdebug')
        dbg('~USING DEBUG config file: %s/config' % terminator_config_dir)
    else:
        terminator_config_dir = os.path.join(configdir, 'terminator')
    return terminator_config_dir

...
class Paned(Container):
    ...
    def split_axis(self, widget, vertical=True, cwd=None, sibling=None, widgetfirst=True):
        """Axis splitter for already Paned."""
        order = None
        dbg('SPLIT PANE FROM: %s of %s [%s]' % (widget.config_section, self.config_section, sibling))
        profile = self.config.get_term_config(self.config_section, widget)
        profile['caption'] = self.caption
    ...
    def set_caption(self, capt):
        ...
        if not capt:
            capt = 'NC' # Paned must have
        self.caption = capt
        self.config.commit(self.config_section, 'caption', capt)
        # do not descend in children, they must have our caption anyway
        self.get_child1().grab_focus() # make focus to terminal

    def create_layout(self, layout):
        """Apply layout configuration"""
        if not self.config_section:
            err('no config_section for %s of %s.' % (layout['type'], layout['parent']))
            return
        if not layout.has_key('children'):
            err('layout specifies no children: %s' % layout)
            return
        children = layout['children']
        if len(children) != 2: # Paned widgets can only have two children
            err('incorrect number of children for Paned: %s' % layout)
            return
        if self.caption: # parent gave us one already
            pass
        elif layout.has_key('caption'):
            self.caption = layout['caption']
        else:
            self.caption = 'NC' # Paned tree tab must have a caption

        dbg('CREATE LAYOUT for %s:%s [%s] of %s.' % (layout['type'], self.config_section, self.caption, layout['parent']))
        keys = []
        keys = children.keys()
        keys.sort()
        num = 0
        for child_key in keys:
            ...
            dbg('Making child of type: %s [%s]' % (child['type'], self.caption))
        ...
        c0 = self.get_child1()
        c0.inPaned = True
        c0.caption = self.caption
        c0.config_section = keys[0]
        c0.create_layout(children[keys[0]])
        c1 = self.get_child2()
        c1.inPaned = True
        c1.caption = self.caption
        c1.config_section = keys[1]
        c1.create_layout(children[keys[1]])

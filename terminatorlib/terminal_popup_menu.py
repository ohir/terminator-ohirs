...
class TerminalPopupMenu(object):
    ...
    def show(self, widget, event=None):
        ...
        if not self.terminator.doing_prefs:
        ...
        item = Gtk.CheckMenuItem.new_with_mnemonic(_('Hide ta_bbar'))
        item.set_active(self.config['tabs_hidden'])
        item.connect('toggled', lambda x: terminal.do_tabshidden_toggle())
        menu.append(item)

        item = Gtk.MenuItem.new_with_mnemonic(_('Save _Layout'))
        item.connect('activate', lambda x: terminal.emit('layout-save'))
        menu.append(item)

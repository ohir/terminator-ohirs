...
#        err('Unable to load Keybinder module. This means the hide_window shortcut will be unavailable')
...
#                self.set_wmclass(options.classname, self.wmclass_class)
    ...
    def get_custom_title(self):
        return self.title.get_custom_title()

    def set_custom_title(self, newtext):
        self.title.force_title(newtext)

    def get_custom_fixed(self):
        return self.title.fixed

    def set_custom_fixed(self, val=True):
        self.title.fixed = val
        self.title.update()
...
#                    self.hidefunc = self.hide
    ...
    def on_destroy_event(self, widget, data=None):
        ...
        dbg('~destroying self %s' % widget)
        self.terminator.doing_quit = True
        for terminal in self.get_visible_terminals():
            terminal.close()
    ...
    def split_axis(self, widget, vertical=True, cwd=None, sibling=None, widgetfirst=True):
        ...
        dbg('~SPLIT WINDOW: %s of %s [%s]' % (widget.config_section, self.config_section, sibling))
        order = None
        maker = Factory()
        ...
        capt = self.title.get_custom_title()
        if capt == '':
            capt = 'NC'
        ...
        if not sibling:
            ...
            # FIXME why force profile if we got configured sibling?
            sibling.force_set_profile(None, widget.get_profile())

        container.caption = capt
        widget.caption = capt
...
(fwidth:%s)(fheight:%s)' % (extra_width, extra_height,
    ...
    def create_layout(self, layout):
        ...
        if not hasattr(self, 'config_section'):
            err('Err config_section is unknown %s' % layout)
            return
        ...
        child_key = children.keys()[0] # Window has only one child
        child = children[child_key]
        child['_config_section'] = child_key
        ...
        c0 = self.get_children()[0]
        c0.config_section = child_key
        c0.create_layout(child)
...
class WindowTitle(object):
    ...
    forced = False
    custom = ''
    fixed = True # do not add os title after custom one
    ...
    def get_custom_fixed(self, val=True):
        self.fixed = val
    ...
    def update(self):
        ...
        if self.forced and not self.fixed:
            title = "%s::%s" % (self.custom, self.text)
        elif self.forced:
            title = self.custom
        else:
            title = "%s" % self.text

...
from util import dbg, err, spawn_new_terminator, make_uuid \
    , manual_lookup, display_manager, get_home_dir
import util
...
class Terminal(Gtk.VBox):
    ...
    __gsignals__ = {
        ...
        'layout-save': (GObject.SignalFlags.RUN_LAST, None, ()),
    ...
    def __init__(self):
        ...
        self.connect('layout-save', self.terminator.save_yourself)
        ...
        #self.connect('title-change', self.titlebar.update_terminal_title)
        self.connect('title-change', self.on_title_change)
    ...
    def set_profile(self, _widget, profile, force=False):
        ...
        if profile != self.config.get_profile():
            ...
            self.config.commit(self.config_section, 'profile', profile)
            self.reconfigure()
    ...
    def do_tabshidden_toggle(self):
        self.config['tabs_hidden'] = not self.config['tabs_hidden']
        window = self.get_toplevel()
        nb = self.get_toplevel().get_children()[0]
        maker = Factory()
        if maker.isinstance(nb, 'Notebook'):
            nb.configure()
    ...
    def set_caption(self, capt):
        """Parent notebook gave us a new caption, we fill config"""
        self.caption = capt
        self.update_subwidgets()
        self.config.commit(self.config_section, 'caption', capt)
        self.vte.grab_focus()

    def on_edit_done(self, _widget):
        """A tabbar is done editing a label, return focus to VTE"""
        self.from_subwidgets()
        self.config.commit(self.config_section, 'title', self.title)
        self.vte.grab_focus()

    def on_title_change(self, widget, title): # 'title-change' handler
        #dbg('~CHANGE %s title: %s f[%d]' % (title, self.config_section, self.dirfixed))
        if not self.dirfixed:
            cwd = self.get_cwd()
            if self._cwdprev != cwd:
                self._cwdprev = cwd
                self.config.commit(self.config_section, 'directory', cwd)
        self.titlebar.update_terminal_title(widget, title)
    ...
    def spawn_child(self, widget=None, respawn=False, debugserver=False):
        ...
        elif self.term_command:
            if self.term_command[0] == '!' and len(self.term_command) > 1:
                fcmd = "%s" % self.term_command[1:] # Bang! just fill line
            else:
                fcmd = "%s\n" % self.term_command # execute in child
        elif self.config['use_custom_command']:
            command = self.config['custom_command'] # profile's command
        ...
        envv = []
        if self.term_envlist:
            for v in self.term_envlist:
                envv.append(v)
        if self.histfile:
            envv.append('HISTFILE=%s/%s' % (get_home_dir(), self.histfile))
        envv.append('TERM=%s' % self.config['term'])
        envv.append('COLORTERM=%s' % self.config['colorterm'])
        envv.append('PWD=%s' % self.cwd)
        envv.append('TERMINATOR_CFG=%s' % self.config_section)
        ...
        dbg('EXE Forking shell: "%s" with args: %s' % (shell, args))
        args.insert(0, shell)
        ...
        dbg('EXE Forked shell: "%s" with args: %s' % (shell, args))
        if self.envfile:
            fenv = "%s/%s" % (get_home_dir(), self.envfile)
            if os.access(fenv, os.R_OK):            # .envSmth Smth.env => SMTH
                self.envfname = self.envfile.replace('.env','').upper()
                #self.envfname = self.envfile.replace('.env','')
                fenv = "source %s/%s\n" % (get_home_dir(), self.envfile)
                self.feed(fenv)
            else:
                self.envfname = 'INVALID'
            self.update_subwidgets()
        else:
            self.envfname = ''
        if fcmd:
        ...
        self.terminator.layout_changed()
...
%s plugin' % urlplugin.handler_name)
    ...
    def describe_layout(self, parent, global_layout):
        ...
        if self.zombie:
            return False
        layout = {}
        name = "%st" % self.config.name_next()
        # name = self.config.name_next()
        if not self.config_section:
            dbg('~TERMINAL EMPTY at describe self new[%s]' % name)
        self.config_section = name
        global_layout[name] = layout
        dbg('~DESCRIBE  [Terminal:%s of %s]' % (name, parent))
        self.from_subwidgets()
        layout['type'] = 'Terminal'
        layout['parent'] = parent
        layout['term_command'] = self.term_command
        layout['term_env'] = self.term_env
        layout['title'] = self.title # bar.get_custom_title()
        layout['titlefixed'] = self.titlefixed
        layout['dirfixed'] = self.dirfixed
        layout['_lastwdir'] = self.get_cwd()
        if self.dirfixed:
            layout['directory'] = self.directory
        else:
            layout['directory'] = layout['_lastwdir']
        layout['profile'] = self.profile #  self.get_profile()
        layout['caption'] = self.caption
        layout['_inPaned'] = self.inPaned
        layout['histfile'] = self.histfile
        if self.histfile:
            layout['histfile'] = self.histfile
            layout['_histfile'] = self.histfile
            layout['_histcust'] = True
            self._sufix = self.histfile.split('.')[-1]
            dbg('LAY got sufix: %s' % self._sufix)
        else:
            if not self._sufix:
                self._sufix = str(self.uuid).split('-')[0]
            layout['_histfile'] = '.term.history.%s' % self._sufix
            layout['_histcust'] = False
        if self.envfile:
            layout['envfile'] = self.envfile
            layout['_envfile'] = self.envfile
            layout['_envcust'] = True
        else:
            layout['_envfile'] = 'custom.env'
            layout['_envcust'] = False
        if self.group:
            layout['group'] = self.group
        dbg('DESCribed [Terminal:%s]' % self.config_section)
        return(True)

    def from_subwidgets(self):
        self.title = self.titlebar.get_custom_title()
        self.profile = self.get_profile()

    def update_subwidgets(self):
        self.titlebar.set_custom_title(self.title)
        self.titlebar.titlefixed = self.titlefixed
        self.titlebar.set_custom_caption(self.caption)
        self.titlebar.set_custom_env(self.envfname)
        if self.profile != self.get_profile():
            if self.profile in self.config.list_profiles():
                self.set_profile(self, self.profile)
        self.titlebar.update()

    def forclone(self):
        """Describe our layout for clone"""
        dbg('~DESCRIBE FOR CLONE from terminal %s.' % self.config_section)
        layout = {}
        self.from_subwidgets()
        layout['type'] = 'Terminal'
        layout['term_command'] = self.term_command
        ...
        layout['titlefixed'] = self.titlefixed
        layout['dirfixed'] = self.dirfixed
        layout['profile'] = self.profile
        layout['caption'] = self.caption
        layout['histfile'] = self.histfile
        layout['envfile'] = self.envfile
        layout['group'] = self.group
        layout['_tabnext'] = self.get_tabnum() + 1
        if self.dirfixed:
            layout['directory'] = self.directory
        else:
    ...
    def get_tabnum(self):
        w = self.get_toplevel()
        if w.is_child_notebook():
            return w.get_children()[0].get_current_page()
        else:
            return (-1)

    def apply_new(self, layout):
        """apply cloned layout"""
        if not layout: # BAAD or self.terminator.doing_layout:
            dbg('~NOT APPLY for %s [layout empty]' % self.config_section)
            return
        dbg('~APPLY LAYOUT for %s' % self.config_section)
        self.title = layout['title']
        self.titlefixed = layout['titlefixed']
        self.caption = layout['caption']
        self.profile = layout['profile']
        self.dirfixed = layout['dirfixed']
        self.directory = layout['directory']
        self.term_command = layout['term_command']
        self.term_env = layout['term_env']
        self.histfile = layout['histfile']
        self.envfile = layout['envfile']
        self.update_subwidgets()

    def relayout(self, global_layout):
        """REapply our layout"""
        ## All keys should have been present as filled by the describe_layout.
        ## We asume valid global_layout and:
        # self.config_section - set by parent relayout
        # self.inPaned        - set by parent relayout
        layout = global_layout[self.config_section]
        dbg('RELAYOUT for %s' % self.config_section)
        self.title = layout['title']
        self.titlefixed = layout['titlefixed']
        self.caption = layout['caption']
        self.profile = layout['profile']
        ...
        self.term_command = layout['term_command']
        self.term_env = layout['term_env']
        # we do not create group here
        if layout['_histcust']:
            self.histfile = layout['histfile']
        else:
            self.histfile = '' # FIXME should we delete file?
        if layout['_envcust']:
            self.envfile = layout['envfile']
        else:
            self.envfile = ''
        self.update_subwidgets()
        dbg('END RELAYOUT for %s' % self.config_section)

    # FIXME here we need to check for key's existence until config will
    # take care of them all
    def create_layout(self, layout):
        """Apply our layout"""
        if not self.config_section:
            err('XXY config_section is empty')
            return
        if not layout:
            err('layout is empty')
            return
        dbg('XXY create_layout for Terminal:%s' % self.config_section)
        if self.caption: # parent gave us one already
            pass
        elif layout.has_key('caption') and layout['caption'] != '':
            self.caption = layout['caption']
        if layout.has_key('title') and layout['title'] != '':
            self.title = layout['title']
            #self.titlebar.set_custom_title(layout['title'])
        if layout.has_key('titlefixed') and layout['titlefixed'] == 'True':
            self.titlefixed = True
            #self.titlebar.titlefixed = True
        if layout.has_key('directory') and layout['directory'] != '':
            self.directory = layout['directory']
        if layout.has_key('term_command') and layout['term_command'] != '':
            self.term_command = layout['term_command']
        if layout.has_key('historysufix') and layout['historysufix'] != '':
            self.historysufix = layout['historysufix']
        if layout.has_key('dirfixed') and layout['dirfixed'] == 'True':
            self.dirfixed = True
        if layout.has_key('histfile') and layout['histfile'] != '':
            self.histfile = layout['histfile']
        if layout.has_key('envfile') and layout['envfile'] != '':
            self.envfile = layout['envfile']
        ...
        if layout.has_key('group') and layout['group'] != '':
            # delayed creation safer than sending None
            self.really_create_group(self.titlebar, layout['group'])

        self.update_subwidgets()
        dbg('END create_layout for Terminal:%s' % self.config_section)
    ...
    def key_new_window(self):
        self.terminator.new_window(self.get_cwd(), self.forclone())

    def key_new_tab(self):
        self.get_toplevel().tab_new(self, self)
        #self.get_toplevel().tab_new(self)

    def key_hide_tab_bar(self):
        self.do_tabshidden_toggle()
    ...
    def key_edit_window_title(self):
        ...
        # FIXME account for added fixed/custom flags

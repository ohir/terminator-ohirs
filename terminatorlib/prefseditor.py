...
# DO NOT EVER MAKE a new Config object until config.py will take a massive
# refactoring. Use config that was populated by the terminator init.
class PrefsEditor:
...
#83a598:#d3869b:#8ec07c:#ebdbb2'}
    ...
    def __init__ (self, term):
        self.term = term
        self.tm = Terminator()
        self.tm.doing_prefs = True
        self.tm.layout_readcurrent()
        self.config = self.tm.config # DONT DO self.config = config.Config(),
        # self.config.base.reload() Now I know why someone did this (ohir)
        # self.undo = self.config.base.get_undo_tree() # XXX future feature
        ...
        self.window.connect('destroy', self.on_cancelbutton_clicked)
        self.layouteditor.prepare()
        self.window.show_all()
        try:
            dbg('~prefseditor: FORBID SAVES')
            self.tm.config.set_nosave(True)
            self.set_values()
        except Exception, e:
            err('Unable to set values: %s' % e)

    ## FIXME we need to make our instance on config to the global
    ## then we need to reconfigure also our tm children
    def on_closebutton_clicked(self, _button, data=None):
        """Redo changes, close the window"""
        self.tm.relayout()
        self.tm.reconfigure()
        dbg('~prefseditor: ENABLE SAVES (on_closebutton_clicked)')
        self.tm.config.set_nosave(False)
        self.tm.save_yourself()
        self._closed = True # both [Cancel] and [x] are bond to on_cancelbutton_clicked
        self.window.destroy()
        self.tm.doing_prefs = False
        del(self)
    ...
    def on_cancelbutton_clicked(self, _button):
        ...
        if self._closed:
            return
        self.tm.layout_readcurrent()
        dbg('~prefseditor: ENABLE SAVES (on_cancelbutton_clicked)')
        self.tm.config.set_nosave(False)
        self.window.destroy()
        self.tm.doing_prefs = False
        del(self)
    ...
    def set_values(self):
        """Update the preferences window with all the configuration
        from our terminator config."""
        ...
        # hide_tabbar
        widget = guiget('tabshiddencheck')
        widget.set_active(self.config['tabs_hidden'])
        # homogeneous_tabbar
        widget = guiget('homogeneouscheck')
        widget.set_active(self.config['homogeneous_tabbar'])
        # do_not_save_dotnew
        widget = guiget('do_not_save_dotnew_checkbox')
        widget.set_active(self.config['do_not_save_dotnew'])
        # DBus Server
        ...
        #Set no user@host in the bar
        widget = guiget('title_hide_user_check')
        widget.set_active(self.config['title_hide_userhost'])
        #Set tab caption in the bar
        widget = guiget('title_hide_tabcap_check')
        widget.set_active(self.config['title_hide_tabcaption'])
        #Set short title in the bar
        widget = guiget('title_hide_path_check')
        widget.set_active(self.config['title_hide_path'])
        ...
        for layout in layouts:
            # if layout == 'default' or layout == 'current':
            if layout == 'default':
                editable = False
            else:
                editable = True
    ...
    def do_not_save_dotnew_toggled(self, widget):
        """do_not_save_dotnew setting changed"""
        self.config['do_not_save_dotnew'] = widget.get_active()
        self.config.set_dirty()
    ...
    def on_title_hide_user_check_toggled(self, widget):
        """No user@host in bar title setting changed"""
        self.config['title_hide_userhost'] = widget.get_active()
        self.config.set_dirty()

    def on_title_hide_tabcap_check_toggled(self, widget):
        """No tab caption in bar title setting changed"""
        self.config['title_hide_tabcaption'] = widget.get_active()
        self.config.set_dirty()

    def on_title_hide_path_check_toggled(self, widget):
        """No path in bar title setting changed"""
        self.config['title_hide_path'] = widget.get_active()
        self.config.set_dirty()
    ...
    def on_tabshidden_toggled(self, widget):
        """tabs_hidden_ setting changed"""
        self.config['tabs_hidden'] = widget.get_active()
        self.config.set_dirty()
    ...
    def on_layoutrefreshbutton_clicked(self, _button):
        ...
        self.tm.relayout()
        dbg('~prefseditor: ENABLE SAVES (on_layoutrefreshbutton_clicked)')
        self.tm.config.set_nosave(False)
        self.tm.save_yourself()
        self.tm.config.set_nosave(True)
        dbg('~prefseditor: FORBID SAVES (on_layoutrefreshbutton_clicked)')
        """
        guiget = self.builder.get_object
        treeview = guiget('layoutlist')
        selected = treeview.get_selection()

        if selected == tm.layoutname:
            self.config.set_dirty()
        else:
            self.tm.save_yourself()
        """
    ...
    def on_layout_profile_tabcaption_changed(self, widget):
        """A different tab caption has been entered for this item"""
        self.layouteditor.on_layout_profile_tabcaption_activate(widget)

    def on_layout_profile_tabbartitle_changed(self, widget):
        """A different tab bar title has been entered for this item"""
        self.layouteditor.on_layout_profile_tabbartitle_activate(widget)

    def on_layout_profile_titlefixed_toggled(self, widget):
        """A tab bar title fixed has been toggled"""
        self.layouteditor.on_layout_profile_titlefixed_activate(widget)

    def on_layout_profile_directory_changed(self, widget):
        """A different working directory has been entered for this item"""
        self.layouteditor.on_layout_profile_directory_activate(widget)

    def on_layout_profile_dirfixed_toggled(self, widget):
        """Use directory entry and make it to config"""
        self.layouteditor.on_layout_profile_dirfixed_activate(widget)

    def on_layout_profile_histfile_changed(self, widget):
        """A different working directory has been entered for this item"""
        self.layouteditor.on_layout_profile_histfile_activate(widget)

    def on_layout_profile_histfixed_toggled(self, widget):
        """Use directory entry and make it to config"""
        self.layouteditor.on_layout_profile_histfixed_activate(widget)

    def on_layout_profile_envfile_changed(self, widget):
        """A different working directory has been entered for this item"""
        self.layouteditor.on_layout_profile_envfile_activate(widget)

    def on_layout_profile_envfixed_toggled(self, widget):
        """Use directory entry and make it to config"""
        self.layouteditor.on_layout_profile_envfixed_activate(widget)
...
class LayoutEditor:
    ...
    def set_layout(self, layout_name):
        ...
        children = sorted(layout.keys(), reverse=True)
        while children != []:
            child = children.pop()
            child_type = layout[child]['type']
            parent = layout[child]['parent']

            if child_type != 'Window' and child_type != 'Defstub' and parent not in layout:
                # We have an orphan!
            ...
            try:
                ...
                if child_type == 'Window' or child_type == 'Defstub':
                    parentiter = None
                else:
            ...
            elif child_type == 'Defstub':
                child_type = 'New defaults'
    ...
    def on_layout_selection_changed(self, selection):
        ...
        widget = guiget('layoutremovebutton')
        if layout == 'default':
            widget.set_sensitive(False)
        else:
            widget.set_sensitive(True)
        ...
        chooser = guiget('layout_profile_chooser')
        command = guiget('layout_profile_command')
        tabcapt = guiget('layout_profile_tabcaption')
        tabbati = guiget('layout_profile_tabbartitle')
        btitfix = guiget('layout_profile_titlefixed')
        workdir = guiget('layout_profile_directory')
        wdirfix = guiget('layout_profile_dirfixed')
        hisfile = guiget('layout_profile_histfile')
        histfix = guiget('layout_profile_histfixed')
        envfile = guiget('layout_profile_envfile')
        envfix  = guiget('layout_profile_envfixed')
        chooser.set_sensitive(False)
        command.set_sensitive(False)
        tabcapt.set_sensitive(False)
        tabbati.set_sensitive(False)
        btitfix.set_sensitive(False)
        workdir.set_sensitive(False)
        wdirfix.set_sensitive(False)
        hisfile.set_sensitive(False)
        histfix.set_sensitive(False)
        envfile.set_sensitive(False)
        envfix.set_sensitive(False)
    ...
    ## FIXME layout_profile_* is misleading, these are for leaf objects
    ## but sound like for profiles tab. Need also fix in preferences.glade
    def set_layout_item(self, item_name):
        ...
        latype = layout_item['type']
        dbg('selected: %s' % latype)
        # clear widgets
        chooser = self.builder.get_object('layout_profile_chooser')
        chooser.set_sensitive(False)
        ...
        command.set_sensitive(False)
        command.set_text('')
        tabcapt = self.builder.get_object('layout_profile_tabcaption')
        tabcapt.set_sensitive(False)
        tabcapt.set_text('')
        tabbati = self.builder.get_object('layout_profile_tabbartitle')
        tabbati.set_sensitive(False)
        tabbati.set_text('')
        btitfix = self.builder.get_object('layout_profile_titlefixed')
        btitfix.set_sensitive(False)
        btitfix.set_active(False)
        workdir = self.builder.get_object('layout_profile_directory')
        workdir.set_sensitive(False)
        workdir.set_text('')
        wdirfix = self.builder.get_object('layout_profile_dirfixed')
        wdirfix.set_sensitive(False)
        wdirfix.set_active(False)
        hisfile = self.builder.get_object('layout_profile_histfile')
        hisfile.set_sensitive(False)
        hisfile.set_text('')
        histfix = self.builder.get_object('layout_profile_histfixed')
        histfix.set_sensitive(False)
        histfix.set_active(False)
        envfile = self.builder.get_object('layout_profile_envfile')
        envfile.set_sensitive(False)
        envfile.set_text('')
        envfix = self.builder.get_object('layout_profile_envfixed')
        envfix.set_sensitive(False)
        envfix.set_active(False)

        # describe_layout made all needed keys, possible '' empty
        # get rid of all has_key.

        if latype == 'VPaned' or latype == 'HPaned':
            tabcapt.set_text(layout_item['caption'])
            if not layout_item['_inPaned']:
                tabcapt.set_sensitive(True)
            self.is_populating = False
        elif latype == 'Window':
            tabbati.set_text(layout_item['title'])
            tabbati.set_sensitive(True)
            btitfix.set_active(layout_item['titlefixed'])
            btitfix.set_sensitive(True)
            self.is_populating = False
        elif latype == 'Defstub':
            if layout_item['profile']:
                chooser.set_active(self.profile_profile_to_ids[layout_item['profile']])
            else:
                chooser.set_active(0)
            chooser.set_sensitive(True)
            command.set_text(layout_item['term_command'])
            ...
            tabcapt.set_text(layout_item['caption'])
            tabcapt.set_sensitive(True)
            entry = layout_item['directory']
            if entry:
                wdirfix.set_active(True)
                wdirfix.set_sensitive(True)
                workdir.set_text(entry)
                ...
                workdir.set_text('')
                workdir.set_sensitive(False)
            entry = layout_item['histfile']
            if entry:
                histfix.set_active(True)
                histfix.set_sensitive(True)
                hisfile.set_text(entry)
                hisfile.set_sensitive(True)
            else:
                histfix.set_active(False)
                histfix.set_sensitive(True)
                hisfile.set_text('')
                hisfile.set_sensitive(False)
            entry = layout_item['envfile']
            if entry:
                envfix.set_active(True)
                envfix.set_sensitive(True)
                envfile.set_text(entry)
                envfile.set_sensitive(True)
            else:
                envfix.set_active(False)
                envfix.set_sensitive(True)
                envfile.set_text('')
            ...
            if not layout_item['_inPaned']:
                tabcapt.set_sensitive(True)
            if layout_item['dirfixed'] == True:
                wdirfix.set_active(True)
                workdir.set_text(layout_item['directory'])
                workdir.set_sensitive(True)
            else:
                wdirfix.set_active(False)
                workdir.set_text(layout_item['_lastwdir'])
                # grayed out
            wdirfix.set_sensitive(True)

            if layout_item['_histcust'] == True:
                histfix.set_active(True)
                hisfile.set_text(layout_item['histfile'])
                hisfile.set_sensitive(True)
            else:
                hisfile.set_text('shell\'s default')
                hisfile.set_sensitive(False)
            histfix.set_sensitive(True)

            if layout_item['_envcust'] == True:
                envfix.set_active(True)
                envfile.set_text(layout_item['envfile'])
                envfile.set_sensitive(True)
            else:
                envfile.set_text('shell\'s default')
                envfile.set_sensitive(False)
            envfix.set_sensitive(True)
        ...
        #self.builder.get_object('cancelbutton').set_sensitive(True)

    def on_layout_profile_chooser_changed(self, widget):
        """A new profile has been selected for this item"""
        if self.is_populating or not self.layout_item:
            return
    ...
    def on_layout_profile_command_activate(self, widget):
        """A new term_command has been entered for this item"""
        if self.is_populating or not self.layout_item:
            return
        term_command = widget.get_text()
        layout = self.config.layout_get_config(self.layout_name)
        layout[self.layout_item]['term_command'] = term_command
        self.config.set_dirty()

    def on_layout_profile_tabcaption_activate(self, widget):
        """A new tab caption has been entered for this item"""
        if self.is_populating or not self.layout_item:
            return
        tabcapt = widget.get_text()
        layout = self.config.layout_get_config(self.layout_name)
        layout[self.layout_item]['caption'] = tabcapt
        self.config.set_dirty()

    def on_layout_profile_tabbartitle_activate(self, widget):
        """A new tab bar title has been entered for this item"""
        if self.is_populating or not self.layout_item:
            return
        tabbati = widget.get_text()
        layout = self.config.layout_get_config(self.layout_name)
        layout[self.layout_item]['title'] = tabbati
        self.config.set_dirty()

    def on_layout_profile_titlefixed_activate(self, widget):
        """keep bar title short changed"""
        if self.is_populating or not self.layout_item:
            return
        layout = self.config.layout_get_config(self.layout_name)

        if widget.get_active() == True:
            layout[self.layout_item]['titlefixed'] = True
        else:
            layout[self.layout_item]['titlefixed'] = False

        self.config.set_dirty()

    def on_layout_profile_directory_activate(self, widget):
        """A new working directory has been entered for this item"""
        if self.is_populating or not self.layout_item:
            return
        layout = self.config.layout_get_config(self.layout_name)
        if layout[self.layout_item]['dirfixed'] == True:
            layout[self.layout_item]['directory'] = widget.get_text()
            self.config.set_dirty()

    def on_layout_profile_dirfixed_activate(self, widget):
        """Make current working dir permanent in config"""
        if self.is_populating or not self.layout_item:
            return
        layout = self.config.layout_get_config(self.layout_name)
        layout[self.layout_item]['dirfixed'] = widget.get_active()
        workdir = self.builder.get_object('layout_profile_directory')
        if widget.get_active() == True:
            # cycle through old-set, new-set and current. User might clicked
            # accidentally so lets her undo this way
            layout[self.layout_item]['_lastwdir'] \
                = layout[self.layout_item]['directory']
            workdir.set_sensitive(True)
            layout[self.layout_item]['directory'] = workdir.get_text()
        else:
            workdir.set_text(layout[self.layout_item]['_lastwdir'])
            workdir.set_sensitive(False)
        self.config.set_dirty()

    def on_layout_profile_histfile_activate(self, widget):
        """A new HISTFILE path has been entered"""
        if self.is_populating or not self.layout_item:
            return
        layout = self.config.layout_get_config(self.layout_name)
        if layout[self.layout_item]['_histcust'] == True:
            layout[self.layout_item]['histfile'] = widget.get_text()
            self.config.set_dirty()

    def on_layout_profile_histfixed_activate(self, widget):
        """Turn on/off custom history file"""
        if self.is_populating or not self.layout_item:
            return
        layout = self.config.layout_get_config(self.layout_name)
        layout[self.layout_item]['_histcust'] = widget.get_active()
        hisfile = self.builder.get_object('layout_profile_histfile')
        if widget.get_active() == True:
            hf = layout[self.layout_item]['_histfile']
            hisfile.set_text(hf)
            layout[self.layout_item]['histfile'] = hf
            hisfile.set_sensitive(True)
        else:
            layout[self.layout_item]['_histfile'] = hisfile.get_text()
            layout[self.layout_item]['histfile'] = ''
            hisfile.set_text('shell\'s default') # FIXME this should be translatable
            hisfile.set_sensitive(False)
        self.config.set_dirty()

    def on_layout_profile_envfile_activate(self, widget):
        """A new ENVFILE path has been entered"""
        if self.is_populating or not self.layout_item:
            return
        layout = self.config.layout_get_config(self.layout_name)
        if layout[self.layout_item]['_envcust'] == True:
            layout[self.layout_item]['envfile'] = widget.get_text()
            self.config.set_dirty()

    def on_layout_profile_envfixed_activate(self, widget):
        """Turn on/off environment customization"""
        if self.is_populating or not self.layout_item:
            return
        layout = self.config.layout_get_config(self.layout_name)
        layout[self.layout_item]['_envcust'] = widget.get_active()
        envfile = self.builder.get_object('layout_profile_envfile')
        if widget.get_active() == True:
            hf = layout[self.layout_item]['_envfile']
            envfile.set_text(hf)
            layout[self.layout_item]['envfile'] = hf
            envfile.set_sensitive(True)
        else:
            layout[self.layout_item]['_envfile'] = envfile.get_text()
            layout[self.layout_item]['envfile'] = ''
            envfile.set_text('shell\'s default') # FIXME this should be translatable
            envfile.set_sensitive(False)
        self.config.set_dirty()
